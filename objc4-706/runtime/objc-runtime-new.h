...
struct method_t {
    SEL name;           //方法的名字
    const char *types;  //Type Encoding类型编码
    IMP imp;            //IMP是一个函数指针，指向的是函数的具体实现
...
struct class_rw_t { //rw-readwrite，ro-readonly
    // Be warned that Symbolication knows the layout of this structure.
    uint32_t flags;
    uint32_t version;

    const class_ro_t *ro;           //是一个指向常量的指针，存储属性、方法和遵守协议
...
struct objc_class : objc_object {
    // Class ISA;
    Class superclass;
    cache_t cache;             // formerly cache pointer and vtable       //方法地址缓存、vtable
    class_data_bits_t bits;    // class_rw_t * plus custom rr/alloc flags
    /*
     class_data_bits_t相当于 class_rw_t指针加上 rr/alloc 的标志
     没有缓存时，从这里查找方法，objc_class的data()函数或者class_data_bits_t的data()函数可以获取到class_rw_t指针
      方法列表存在class_rw_t指针中。
     
     在编译期，class_data_bits_t的bits属性指向的是一个class_ro_t *指针
