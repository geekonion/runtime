...
#   define ISA_MAGIC_VALUE 0x000001a000000001ULL
    struct {
        uintptr_t nonpointer        : 1;                                        //原来是index:代表是否开启isa指针优化。index = 1，代表开启isa指针优化\
                                                                                目前和原来意义一样吗？
        uintptr_t has_assoc         : 1;                                        //对象含有或者曾经含有关联引用，没有关联引用的可以更快地释放内存
        uintptr_t has_cxx_dtor      : 1;                                        //表示该对象是否有 C++ 或者 Objc 的析构器
        uintptr_t shiftcls          : 33; // MACH_VM_MAX_ADDRESS 0x1000000000   //类的指针。arm64架构中有33位可以存储类指针。
        uintptr_t magic             : 6;                                        //判断对象是否初始化完成，在arm64中0x16是调试器判断当前对象是\
                                                                                真的对象还是没有初始化的空间。
        ...
        uintptr_t extra_rc          : 19;                                       //存放该对象的引用计数值减一后的结果
