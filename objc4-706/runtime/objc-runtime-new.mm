...
{
#if SUPPORT_QOS_HACK
    /* 函数原型：pthread_priority_t _pthread_qos_class_encode(qos_class_t qos_class, int relative_priority, unsigned long flags);
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // Discover load methods
    {
        ...
        //把需要调用load方法的类加到loadable_classes数组里，需要调用load方法的分类加到add_category_to_loadable_list数组里
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    /*
    if (strstr(name, "Person")) {
        printf("");
    }*/
//    printf("lookUpImpOrForward--> 类: %s sel: %s\n", name, sel_getName(sel));
