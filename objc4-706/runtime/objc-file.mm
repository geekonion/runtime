...
/*
 用宏定义函数
 例： GETSECT(_getObjc2SelectorRefs,        SEL,             "__objc_selrefs");
 结果：
 得到两个函数
 SEL *_getObjc2SelectorRefs(const headerType *mhdr, size_t *outCount) {
    return getDataSection<SEL>(mhdr, "__objc_selrefs", nil, outCount);
 }
 SEL *_getObjc2SelectorRefs(const header_info *hi, size_t *outCount) {
    return getDataSection<SEL>(hi->mhdr(), "__objc_selrefs", nil, outCount);
 }
 */
...
GETSECT(_getObjc2NonlazyClassList,    classref_t,      "__objc_nlclslist");   //nlcls->nonlazy class
